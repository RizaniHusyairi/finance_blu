...
class KasubbagSpmKontrakVerifikasiController extends Controller
{
    /**
     * Tampilkan daftar antrean verifikasi SPM Kontrak untuk Kasubbag.
     */
    public function index(Request $request)
    {
        $roleName = 'Kepala Subbagian Keuangan dan Tata Usaha';

        // Ambil SPM khusus dari SPP Kontrak dan melibatkan workflow dari Kasubbag
        ...
        $countPending = $processedSpms->where('kasubbagApprovalStatus', 'PENDING')->count();
        $countApprovedMe = $processedSpms->where('kasubbagApprovalStatus', 'APPROVED')->count();
        $countRevisi = $processedSpms->where('kasubbagApprovalStatus', 'REVISION')->count();
        $countSelesai = $processedSpms->where('statusFinal', 'Selesai Diverifikasi')->count();

        $viewSpms = $processedSpms;

        // Terapkan Filter Status Saya (Kasubbag)
        ...
        // Terapkan Filter PPSPM
        if ($request->has('status_ppspm') && $request->status_ppspm !== 'Semua') {
            if ($request->status_ppspm === 'Pending') {
                $viewSpms = $viewSpms->where('ppspmApprovalStatus', 'PENDING');
            } elseif ($request->status_ppspm === 'Approved') {
                $viewSpms = $viewSpms->where('ppspmApprovalStatus', 'APPROVED');
            } elseif ($request->status_ppspm === 'Revisi') {
        ...
        return view('verifikasi_kasubag.spm_kontrak_index', compact(
            'viewSpms',
            'countPending',
            'countApprovedMe',
            'countRevisi',
    ...
    /**
     * Tampilkan halaman detail untuk proses pengambilan keputusan oleh Kasubbag.
     */
    public function show($id)
    {
        ...
        return view('verifikasi_kasubag.spm_kontrak_detail', compact(
            'spmModel',
            'wf',
            'ppspmApproval',
            'kasubbagApproval',
    ...
    public function approve($id)
    {
        ...
        try {
            app(WorkflowService::class)->approveCurrentStep($spm, Auth::id(), 'Dokumen SPM disetujui oleh Kasubbag.');
            ...
            $statusBaru = $workflowFullyApproved ? DokumenSpm::STATUS_SPM_TERBIT : 'Disetujui Kasubbag';
            ...
            LogStatusDokumen::create([
                ...
                'catatan' => $workflowFullyApproved
                    ? 'Dokumen SPM disetujui Kasubbag. SPM terbit ber-TTE QR dan siap dibuatkan NPI.'
                    : 'Dokumen SPM disetujui Kasubbag.',
            ...
            return redirect()->route('verifikasi-kasubag.spm.kontrak.index')->with('success', $workflowFullyApproved
                ? "SPM Nomor {$spm->nomor_spm} telah disetujui oleh semua pihak."
                : "SPM Nomor {$spm->nomor_spm} berhasil disetujui oleh Anda (Kasubbag).");
        } catch (\Exception $e) {
    ...
    public function revisi(Request $request, $id)
    {
        ...
        try {
            ...
            $operators = User::role('Operator BLU')->get();
            Notification::send($operators, new WorkflowNotification([
                'title' => 'SPM Kontrak Direvisi Kasubbag',
                'message' => "SPM {$spm->nomor_spm} perlu revisi. Catatan Kasubbag: {$request->catatan_revisi}",
                'url' => route('spms.kontrak.detail', $spm->spp_id),
                'icon' => 'error_outline',
                'color' => 'danger'
            ]));

            return redirect()->route('verifikasi-kasubag.spm.kontrak.index')->with('warning', "Catatan revisi untuk SPM {$spm->nomor_spm} telah dikirim.");
        } catch (\Exception $e) {
