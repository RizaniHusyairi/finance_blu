...
class BendaharaHonorariumVerifikasiController extends Controller
{
    /**
     * Halaman antrean verifikasi Honorarium untuk Bendahara Pengeluaran.
     */
    public function index(Request $request)
    {
        ...
        $summary = [
            'pending' => $tagihanList->filter(fn ($n) => $n->_bendaharaApproval?->status === 'PENDING')->count(),
            'approved' => $tagihanList->filter(fn ($n) => $n->_bendaharaApproval?->status === 'APPROVED')->count(),
            'revision' => $tagihanList->filter(fn ($n) => in_array($n->_bendaharaApproval?->status, ['REVISION', 'REJECTED'])
                || $n->_workflowInstance?->status === 'REVISION')->count(),
            'selesai' => $tagihanList->filter(fn ($n) => $n->_statusFinal === 'Selesai Diverifikasi')->count(),
        ];

        return view('honorarium.verifikasi.bendahara.index', [
            'tagihanList' => $filtered->values(),
            'summary' => $summary,
            'filterPpk' => $filterPpk,
            'filterBendahara' => $filterBendahara,
            'search' => $search,
            'currentRole' => 'Bendahara Pengeluaran',
            'routePrefix' => 'verifikasi-bendahara.honorarium',
        ]);
    }

    /**
     * Halaman detail verifikasi Honorarium untuk Bendahara Pengeluaran.
     */
    public function show($id)
    {
        ...
        $currentUserApproval = $bendaharaApproval;

        $canApprove = $bendaharaApproval && $bendaharaApproval->status === 'PENDING';
        ...
        return view('honorarium.verifikasi.bendahara.show', [
            'tagihan' => $tagihan,
    ...
    /**
     * Approve Honorarium oleh Bendahara.
     */
    public function approve(Request $request, $id)
    {
        ...
        return redirect()->route('verifikasi-bendahara.honorarium.show', $id)
            ->with('success', 'Honorarium berhasil disetujui.');
    }

    /**
     * Minta revisi Honorarium oleh Bendahara.
     */
        ...
        DB::transaction(function () use ($tagihan, $request) {
            ...
            if ($ppabpUsers->isNotEmpty()) {
                Notification::send($ppabpUsers, new WorkflowNotification([
                    ...
                    'message' => "Honorarium {$tagihan->nomor_tagihan} dikembalikan oleh Bendahara Pengeluaran: {$request->catatan_revisi}",
