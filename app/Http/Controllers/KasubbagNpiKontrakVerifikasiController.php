...
class KasubbagNpiKontrakVerifikasiController extends Controller
{
    /**
     * Halaman antrean verifikasi NPI Kontrak untuk Kasubbag.
     */
    public function index(Request $request)
    {
        ...
        $summary = [
            'pending' => $npiList->filter(fn ($n) => $n->_kasubbagApproval?->status === 'PENDING')->count(),
            'approved' => $npiList->filter(fn ($n) => $n->_kasubbagApproval?->status === 'APPROVED')->count(),
            'revision' => $npiList->filter(fn ($n) => in_array($n->_kasubbagApproval?->status, ['REVISION', 'REJECTED'])
                || $n->_workflowInstance?->status === 'REVISION')->count(),
            'selesai' => $npiList->filter(fn ($n) => $n->_statusFinal === 'Selesai Diverifikasi')->count(),
        ];

        return view('verifikasi_kasubbag.npi_kontrak_index', [
    ...
    /**
     * Halaman detail verifikasi NPI Kontrak untuk Kasubbag.
     */
    public function show($npi_id)
    {
        ...
        $canApprove = $kasubbagApproval && $kasubbagApproval->status === 'PENDING';
        ...
        return view('verifikasi_kasubbag.npi_kontrak_detail', compact(
            'npi',
            'spm',
            'spp',
            'tagihan',
    ...
    public function approve(Request $request, $npi_id)
    {
        ...
        DB::transaction(function () use ($npi, $request) {
            ...
            LogStatusDokumen::create([
                ...
                'catatan' => $request->input('catatan', 'NPI disetujui Kasubbag. NPI terbit ber-TTE dan siap dibuatkan SP2D.'),
        ...
        return redirect()->route('verifikasi-kasubag.npi.kontrak.show', $npi_id)
            ->with('success', 'NPI berhasil disetujui.');
    }

    /**
     * Minta revisi NPI oleh Kasubbag.
     */
        ...
        DB::transaction(function () use ($npi, $request) {
            ...
            if ($benPenUsers->isNotEmpty()) {
                Notification::send($benPenUsers, new WorkflowNotification([
                    'title' => 'NPI Kontrak Dikembalikan oleh Kasubbag',
                    'message' => "NPI {$npi->nomor_npi} dikembalikan oleh Kasubbag: {$request->catatan_revisi}",
