...
@section('content')
    <x-page-title title="Pembuatan SPM" subtitle="Honorarium" />
    ...
    <div class="spm-filter-bar mb-4 d-flex flex-wrap gap-3 align-items-center justify-content-between">
        <div class="d-flex flex-wrap gap-2">
            <a href="{{ route('spms.honor.index', ['status' => 'semua']) }}" class="spm-status-btn {{ $statusFilter === 'semua' ? 'active' : '' }}">Semua</a>
            <a href="{{ route('spms.honor.index', ['status' => 'belum_dibuat', 'search' => $search]) }}" class="spm-status-btn {{ $statusFilter === 'belum_dibuat' ? 'active' : '' }}">Belum Dibuat</a>
            <a href="{{ route('spms.honor.index', ['status' => 'draft', 'search' => $search]) }}" class="spm-status-btn {{ $statusFilter === 'draft' ? 'active' : '' }}">Draft</a>
            <a href="{{ route('spms.honor.index', ['status' => 'revisi', 'search' => $search]) }}" class="spm-status-btn {{ $statusFilter === 'revisi' ? 'active' : '' }}">Revisi</a>
            <a href="{{ route('spms.honor.index', ['status' => 'menunggu', 'search' => $search]) }}" class="spm-status-btn {{ $statusFilter === 'menunggu' ? 'active' : '' }}">Menunggu Verifikasi</a>
            <a href="{{ route('spms.honor.index', ['status' => 'selesai', 'search' => $search]) }}" class="spm-status-btn {{ $statusFilter === 'selesai' ? 'active' : '' }}">Disetujui Final</a>
        </div>
        <form action="{{ route('spms.honor.index') }}" method="GET" class="d-flex gap-2" style="min-width: 280px;">
            <input type="hidden" name="status" value="{{ $statusFilter }}">
            <input type="text" name="search" class="form-control form-control-sm" placeholder="Cari nomor SPM, SPP, tagihan, kegiatan..." value="{{ $search }}">
                        ...
                        @foreach ($sppList as $idx => $spp)
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="text-muted small">Honorarium: {{ $tagihan?->nomor_tagihan ?? '-' }}</div>
                                </td>
                                <td>
                                    <div class="fw-semibold text-wrap">{{ Str::limit($tagihan?->deskripsi ?? '-', 70) }}</div>
                                    <div class="text-muted small">
                                        Penerima: <span class="badge bg-secondary-subtle text-secondary">{{ collect($tagihan?->detailHonorarium)->count() }} Orang</span>
                                    </div>
                                </td>
                                <td class="text-end">
                                ...
                                <td class="text-center">
                                    @if(!$spm)
                                        <a href="{{ route('spms.honor.detail', $spp->id) }}" class="btn btn-sm btn-primary">
                                            <i class="bi bi-plus-circle me-1"></i> Buat SPM
                                        </a>
                                    @elseif(in_array($spm->status, ['DRAFT', 'Revisi']))
                                        <a href="{{ route('spms.honor.detail', $spp->id) }}" class="btn btn-sm btn-warning">
                                            <i class="bi bi-pencil me-1"></i> Lanjutkan
                                        </a>
                                    @else
                                        <a href="{{ route('spms.honor.detail', $spp->id) }}" class="btn btn-sm btn-outline-primary">
        ...
        $(document).ready(function() {
            $('#tabelSpmHonor').DataTable({
                ...
                language: {
                    ...
                    emptyTable: "Tidak ada data SPM Honorarium.",
                }
            });
        });
    </script>
