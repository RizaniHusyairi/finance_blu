@extends('layouts.app')
@section('title', 'Workspace SPM Perjaldin')
...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <x-page-title title="Workspace Operator BLU" subtitle="Detail & Penyusunan SPM Perjaldin" />
        <a href="{{ route('spms.perjaldin.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i> Kembali</a>
    </div>
    ...
    <div class="spm-workspace-hero p-4 rounded-3 shadow-sm">
        <div class="d-flex flex-column flex-xl-row justify-content-between gap-4">
            <div class="flex-grow-1">
                <h3 class="fw-bold mb-2 text-dark">{{ $komponen?->nama_komponen ?? 'SPM Perjaldin' }} — {{ $tagihan?->deskripsi ?? '' }}</h3>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-3 col-6"><div class="spm-summary-tile"><div class="label">Komponen</div><div class="value text-truncate">{{ $komponen?->nama_komponen ?? '-' }}</div></div></div>
            ...
            <div class="d-flex flex-column gap-2" style="min-width: 200px;">
                ...
                <button type="button" class="btn btn-primary shadow-sm" data-bs-toggle="modal" data-bs-target="#modalSpmPerjaldin" {{ $canEditSpm ? '' : 'disabled' }}>
                    <i class="bi bi-pencil-square me-1"></i> {{ $spmModel ? 'Edit Draft SPM' : 'Buat Draft Baru' }}
                </button>

                @if($spmModel)
                    @if($canSubmit && $isReadyToSubmit)
                        <form action="{{ route('spms.perjaldin.submit', $sppModel->id) }}" method="POST" onsubmit="return confirm('Ajukan SPM ini untuk verifikasi PPSPM dan Kasubbag secara paralel?')">
                            @csrf
    ...
    <div class="row g-4">
        ...
        <div class="col-xl-7">
            ...
            <div class="card spm-section-card mb-4">
                <div class="card-body p-4">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6"><div class="spm-info-block"><div class="label">Komponen Biaya</div><div class="value">{{ $komponen?->nama_komponen ?? '-' }}</div></div></div>
                        ...
                        <div class="col-md-6"><div class="spm-info-block"><div class="label">Uraian</div><div class="value">{{ $sppModel->uraian ?? '-' }}</div></div></div>
                    </div>
                </div>
            </div>

            {{-- Ringkasan Tagihan Perjaldin --}}
            <div class="card spm-section-card mb-4">
                <div class="card-body p-4">
                    <div class="spm-section-heading text-primary"><i class="bi bi-airplane"></i> 2. Tagihan Perjaldin</div>
                    <div class="row g-3">
                        ...
                        <div class="col-md-6"><div class="spm-info-block"><div class="label">Deskripsi</div><div class="value">{{ $tagihan?->deskripsi ?? '-' }}</div></div></div>
                        <div class="col-md-4"><div class="spm-info-block"><div class="label">Total Bruto</div><div class="value">Rp {{ number_format($tagihan?->total_bruto ?? 0, 0, ',', '.') }}</div></div></div>
                        <div class="col-md-4"><div class="spm-info-block"><div class="label">Total Potongan</div><div class="value text-danger">Rp {{ number_format(($tagihan?->total_bruto ?? 0) - ($tagihan?->total_netto ?? 0), 0, ',', '.') }}</div></div></div>
                        <div class="col-md-4"><div class="spm-info-block"><div class="label">Total Netto</div><div class="value text-success fs-5">Rp {{ number_format($tagihan?->total_netto ?? 0, 0, ',', '.') }}</div></div></div>
                    </div>
                </div>
            </div>
            ...
            <div class="card spm-section-card mb-4">
                <div class="card-body p-4">
                    <div class="spm-section-heading text-primary"><i class="bi bi-people"></i> 3. Peserta Perjalanan Dinas ({{ $tagihan->detailPerjaldin->count() }} Orang)</div>
            ...
            <div class="card spm-section-card mb-4">
                <div class="card-body p-4">
                    <div class="spm-section-heading text-primary"><i class="bi bi-ui-radios-grid"></i> 4. Rekap Seluruh Komponen Biaya</div>
                    <div class="table-responsive">
                        <table class="table table-sm table-striped align-middle mb-0">
                            <thead class="table-light">
                                <tr>
                            ...
                            <tbody>
                                ...
                                <tr class="{{ $k->id === $komponen?->id ? 'table-primary' : '' }}">
                                    ...
                                    <td class="text-center">
                                        @php $kSpm = $k->dokumenSpp?->spm; @endphp
                                        @if($kSpm)
                                            <span class="badge {{ match($kSpm->status) { 'DRAFT' => 'bg-warning text-dark', 'Menunggu Verifikasi' => 'bg-info', 'Revisi' => 'bg-danger', 'Disetujui Final' => 'bg-success', default => 'bg-secondary' } }}">
                                                {{ match($kSpm->status) { 'DRAFT' => 'Draft', 'Menunggu Verifikasi' => 'Menunggu', 'Revisi' => 'Revisi', 'Disetujui Final' => 'Terbit', default => $kSpm->status } }}
                                            </span>
                                        @else
        ...
        <div class="col-xl-5">
            <div class="sticky-top" style="top: 1.5rem; z-index: 1;">
                ...
                <div class="card spm-section-card mb-4">
                    <div class="card-body p-4">
                        <div class="spm-section-heading text-primary"><i class="bi bi-wallet2"></i> Anggaran & Akun</div>
                        <div class="p-3 bg-light rounded border border-primary border-opacity-25">
                            ...
                            <div class="value fs-5">@if($selectedBudgetItem?->coa) {{ $selectedBudgetItem->coa->kode_mak_lengkap ?? $selectedBudgetItem->coa->kode_akun }} @else <span class="text-danger">Belum Tersedia</span> @endif</div>
    ...
    {{-- MODAL DRAFT SPM --}}
    <div class="modal fade" id="modalSpmPerjaldin" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <form action="{{ route('spms.perjaldin.store', $sppModel->id) }}" method="POST" class="modal-content border-0 shadow">
                @csrf

                <div class="modal-header text-white border-0" style="background: #6366f1;">
                    <h5 class="modal-title fw-bold"><i class="bi bi-pencil-square me-2"></i>{{ $spmModel ? 'Edit Draft SPM Perjaldin' : 'Buat Draft SPM Perjaldin Baru' }}</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>

                <div class="modal-body p-4 bg-light">
                    @if($spmModel && $spmModel->status === 'Revisi')
                        <div class="alert alert-danger mb-4 shadow-sm border-0"><i class="bi bi-exclamation-triangle-fill me-2"></i> Dokumen ini dikembalikan karena memerlukan revisi.</div>
                    ...
                    <fieldset class="border-0 p-0 m-0" {{ $canEditSpm ? '' : 'disabled' }}>
                        ...
                        <div class="spm-modal-section shadow-sm">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6"><label class="form-label fw-semibold">Nomor Tagihan</label><input type="text" class="form-control bg-light" value="{{ $tagihan?->nomor_tagihan ?? '-' }}" readonly></div>
                                <div class="col-md-6"><label class="form-label fw-semibold">Komponen Biaya</label><input type="text" class="form-control bg-light" value="{{ $komponen?->nama_komponen ?? '-' }}" readonly></div>
                                <div class="col-md-6"><label class="form-label fw-semibold">Akun / COA</label><input type="text" class="form-control bg-light" value="{{ $selectedBudgetItem?->coa?->kode_akun ?? '-' }} — {{ $selectedBudgetItem?->coa?->nama_akun ?? '-' }}" readonly></div>
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        @if($errors->any() && old('nomor_spm'))
            const modalElement = document.getElementById('modalSpmPerjaldin');
            if (modalElement) {
