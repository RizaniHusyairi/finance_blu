    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .hero-card {
            ...
            box-shadow: 0 30px 80px rgba(2, 6, 23, .55), inset 0 1px 0 rgba(255,255,255,.06);
        ...
        .chip .dot { width: 7px; height: 7px; border-radius: 50%; background: var(--blue); box-shadow: 0 0 0 0 rgba(56,189,248,.7); animation: pulse 1.9s ease-out infinite; }
        h1 {
            margin: 16px 0 0; font-size: clamp(28px, 4.4vw, 46px); line-height: 1.04; font-weight: 900; letter-spacing: -.02em;
        ...
        .btn {
            display: inline-flex; align-items: center; gap: 9px; min-height: 46px; padding: 0 20px;
            border-radius: 12px; text-decoration: none; font-size: 14px; font-weight: 800; cursor: pointer; border: 1px solid transparent;
        ...
        .trust-card {
            border: 1px solid var(--line); border-radius: 16px; padding: 16px 18px; background: var(--card);
            backdrop-filter: blur(8px); display: flex; align-items: center; gap: 13px;
        }
        .trust-ico { width: 42px; height: 42px; border-radius: 12px; display: grid; place-items: center; flex-shrink: 0; font-size: 19px; }
        ...
        .trust-card .t-lbl { font-size: 11px; font-weight: 800; letter-spacing: .08em; text-transform: uppercase; color: var(--faint); }
        .trust-card .t-val { font-size: 15px; font-weight: 800; margin-top: 2px; }
        ...
        .card {
            ...
            box-shadow: 0 18px 50px rgba(2,6,23,.4); overflow: hidden;
            animation: rise .7s cubic-bezier(.22,1,.36,1) .15s both;
        }
        .card + .card { margin-top: 18px; }
        .card-head { display: flex; align-items: center; gap: 11px; padding: 18px 22px; border-bottom: 1px solid var(--line); }
        .card-head .ch-ico { width: 34px; height: 34px; border-radius: 10px; display: grid; place-items: center; background: rgba(129,140,248,.14); color: var(--indigo); border: 1px solid rgba(129,140,248,.28); font-size: 16px; }
        .card-head h2 { margin: 0; font-size: 15px; font-weight: 800; letter-spacing: -.01em; }
        .card-body { padding: 22px; }

        .scan-strip {
            position: relative; overflow: hidden; display: flex; flex-wrap: wrap; gap: 6px 18px;
            padding: 12px 22px; font-size: 12.5px; color: #9fd5ee; background: rgba(2,6,23,.5); border-bottom: 1px solid var(--line);
            font-family: 'JetBrains Mono', monospace;
        }
        .scan-strip::after { content: ""; position: absolute; top: 0; bottom: 0; width: 90px; left: -100px;
            background: linear-gradient(90deg, transparent, rgba(125,211,252,.25), transparent); animation: scanline 3.4s ease-in-out infinite; }
        ...
        .status { display: inline-flex; align-items: center; gap: 8px; padding: 8px 13px; border-radius: 999px; color: #062b1e; background: linear-gradient(135deg, #6ee7b7, #34d399); font-size: 12.5px; font-weight: 900; }
        .status .dot { width: 7px; height: 7px; border-radius: 50%; background: #04130c; box-shadow: 0 0 0 0 rgba(4,19,12,.6); animation: pulse 1.8s ease-out infinite; }
        ...
        .signer-row .v { font-weight: 750; }

        .hash-box { padding: 16px; border: 1px solid rgba(56,189,248,.22); border-radius: 16px; background: linear-gradient(160deg, rgba(56,189,248,.07), rgba(52,211,153,.04)); }
        ...
        .hash-value { margin-top: 12px; padding: 12px; border-radius: 12px; color: #cfe6ff; background: rgba(2,6,23,.55); border: 1px solid var(--line);
            font-family: 'JetBrains Mono', monospace; font-size: 11.5px; line-height: 1.6; word-break: break-all; }
        ...
        .meta { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1px; background: var(--line); border-top: 1px solid var(--line); }
        .meta-cell { padding: 16px 18px; background: rgba(13,20,38,.6); }
        ...
        @keyframes scanline { 0% { left: -100px; } 55%,100% { left: 100%; } }
        ...
        @media (max-width: 900px) {
            ...
            .trust, .layout, .grid2, .signer, .meta { grid-template-columns: 1fr; }
            .meta { gap: 1px; }
        }
        @media (prefers-reduced-motion: reduce) {
            .aurora span, .hero-card::before, .seal-ring, .seal-core, .scan-strip::after, .chip .dot, .status .dot, .seal-core .check { animation: none !important; }
        ...
        <div class="wrap">
            <div class="hero-card">
                <div>
                    ...
                    <h1>Dokumen {{ $documentLabel }} Terverifikasi &amp; Disetujui PPK</h1>
                    <p class="subtitle">Halaman ini terbit dari QR TTE resmi sistem setelah Pejabat Pembuat Komitmen menyetujui kontrak pengadaan. Keaslian dijamin lewat signed URL &amp; hash dokumen.</p>
                    <div class="actionbar">
            ...
            <div class="trust">
                <div class="trust-card">
                    ...
                    <div>
                        ...
                        <div class="t-val">Sah Disetujui PPK</div>
                    </div>
                </div>
                <div class="trust-card">
                ...
                <div class="trust-card">
                    ...
                    <div>
                        <div class="t-lbl">Ditandatangani</div>
            ...
            <div>
                ...
                <div class="card">
                    <div class="card-head">
                        ...
                        <h2>Ringkasan Dokumen Kontrak</h2>
                    </div>
                    <div class="card-body">
                        <div class="grid2">
                            <div class="field">
                            ...
                            <div class="field">
                                ...
                                <span class="status"><span class="dot"></span> Sah Disetujui PPK</span>
                            </div>
                            <div class="field">
                                <p class="label">Nama Pekerjaan</p>
                                <p class="value">{{ $kontrak->nama_pekerjaan }}</p>
                            </div>
                            <div class="field">
                                <p class="label">Nilai Kontrak</p>
                                <p class="value money">Rp {{ number_format((float) $kontrak->nilai_total_kontrak, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="signer">
                            ...
                            <div class="hash-box">
                                ...
                                <p class="small" style="margin: 11px 0 0;">Hash dihitung dari identitas kontrak dan data persetujuan PPK saat halaman dibuka.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <aside>
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="id-row"><p class="label">ID Kontrak</p><p class="value">{{ $scanInfo['dokumen_id'] }}</p></div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-head">
                        <span class="ch-ico">
                            <svg width="17" height="17" viewBox="0 0 24 24" fill="none"><path d="M3 21h18M5 21V7l8-4 8 4v14M9 9h.01M9 13h.01M9 17h.01M15 9h.01M15 13h.01M15 17h.01" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
                        </span>
                        <h2>Penyedia / Vendor</h2>
                    </div>
                    <div class="card-body">
                        <p class="value" style="font-size: 16px;">{{ $kontrak->vendor?->nama_pihak ?? '-' }}</p>
                        <div class="id-row" style="margin-top: 14px;"><p class="label">Kontrak dibuat</p><p class="value">{{ optional($kontrak->created_at)->format('d M Y H:i') ?? '-' }}</p></div>
                    </div>
                </div>
            </aside>
        </div>
            ...
            <div class="card" style="animation-delay:.2s;">
                <div class="meta">
                    <div class="meta-cell">
                        ...
                        <p class="value">{{ $documentLabel }}-{{ $kontrak->getKey() }}</p>
                    </div>
                    <div class="meta-cell">
                        <p class="label">Nomor SPK</p>
                        <p class="value">{{ $kontrak->nomor_spk ?? '-' }}</p>
                    </div>
                    <div class="meta-cell">
                        <p class="label">Status Kontrak</p>
                        <p class="value">{{ $kontrak->status_kontrak }}</p>
                    </div>
                    <div class="meta-cell">
                        <p class="label">Metode Pembayaran</p>
                        <p class="value">{{ $kontrak->metode_pembayaran }}</p>
                    </div>
                </div>
                <div class="footer-note">
                    ...
                    Tautan ini menggunakan <strong>signed URL</strong> sistem. Perubahan data kontrak atau status persetujuan akan membuat hash tidak cocok sehingga halaman TTE tidak dapat ditampilkan.
                </div>
            </div>
