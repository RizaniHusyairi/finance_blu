@extends('layouts.app')
@section('title', 'Detail Verifikasi SPM Kontrak - Kasubbag')
...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="{{ route('verifikasi-kasubag.spm.kontrak.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i> Kembali ke Antrean</a>
    </div>
                ...
                <div class="card-body p-4">
                    ...
                    @if($latestKasubbagRevisionNote)
                        <div class="alert alert-danger mb-2">
                            <strong class="d-block mb-1">Catatan Anda (Kasubbag):</strong>
                            {{ $latestKasubbagRevisionNote->catatan ?? '-' }}
                        </div>
                    @endif
                    @if($latestPpspmRevisionNote)
                        <div class="alert alert-warning mb-0">
                            <strong class="d-block mb-1">Catatan PPSPM:</strong>
    ...
    @if($canAct)
        <div class="card shadow border-0 bg-white sticky-bottom mb-4" style="bottom: 1rem; z-index: 10;">
            <div class="card-body p-4 d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="fw-bold mb-1">Keputusan Anda (Kasubbag)</h5>
    ...
    @elseif($kasubbagApproval?->status === 'APPROVED')
        <div class="alert alert-success border-0 shadow-sm p-4 d-flex align-items-center mb-4">
             ...
             <div>
                 ...
                 <div>Tanggal Tindakan: {{ optional($kasubbagApproval->acted_at)->format('d M Y H:i') }}</div>
                 <div class="mt-1 small">Status PPSPM saat ini: <strong>{{ $ppspmStatusLabel }}</strong></div>
             </div>
        </div>
    @elseif($kasubbagApproval?->status === 'REVISION')
        <div class="alert alert-danger border-0 shadow-sm p-4 d-flex align-items-center mb-4">
             ...
             <div>
                 ...
                 <div>Tanggal Tindakan: {{ optional($kasubbagApproval->acted_at)->format('d M Y H:i') }}</div>
                 <div class="mt-1 text-dark fst-italic">Catatan: "{{ $latestKasubbagRevisionNote?->catatan ?? '-' }}"</div>
             </div>
        </div>
    @endif
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0">
                ...
                <div class="modal-body p-4 text-center">
                    ...
                    <p class="text-muted mb-0">Tindakan ini akan menyelesaikan dan mengunci tahap verifikasi milik Kasubbag. Pastikan kelengkapan dokumen telah memenuhi syarat.</p>
                </div>
                <div class="modal-footer border-0 justify-content-center pb-4">
                    <button type="button" class="btn btn-light px-4" data-bs-dismiss="modal">Batal</button>
                    <form action="{{ route('verifikasi-kasubag.spm.kontrak.approve', $spmModel->id) }}" method="POST">
                        @csrf
    ...
    <div class="modal fade" id="rejectModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('verifikasi-kasubag.spm.kontrak.revisi', $spmModel->id) }}" method="POST" class="modal-content border-0">
                @csrf
