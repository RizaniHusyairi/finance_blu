@extends('layouts.app')
@section('title')
    Verifikasi SPM Kontrak - Kasubbag
@endsection
@push('css')
...
@section('content')
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body p-3">
            <form action="{{ route('verifikasi-kasubag.spm.kontrak.index') }}" method="GET" class="row g-3 align-items-center">
                <div class="col-md-3">
                    <label class="form-label mb-0 small text-muted">Status Saya (Kasubbag)</label>
                ...
                <div class="col-md-3">
                    <label class="form-label mb-0 small text-muted">Status PPSPM</label>
                    <select name="status_ppspm" class="form-select form-select-sm" onchange="this.form.submit()">
                        <option value="Semua" {{ request('status_ppspm') == 'Semua' ? 'selected' : '' }}>Semua Status</option>
                        <option value="Pending" {{ request('status_ppspm') == 'Pending' ? 'selected' : '' }}>Pending</option>
                        <option value="Approved" {{ request('status_ppspm') == 'Approved' ? 'selected' : '' }}>Approved</option>
                        <option value="Revisi" {{ request('status_ppspm') == 'Revisi' ? 'selected' : '' }}>Revisi</option>
                    </select>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-2 text-end">
                    ...
                    <a href="{{ route('verifikasi-kasubag.spm.kontrak.index') }}" class="btn btn-sm btn-light border">Reset Filter</a>
                </div>
            </form>
        </div>
    </div>
                ...
                <table id="example" class="table table-striped table-bordered align-middle" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @if($spm->canAct)
                                    <a href="{{ route('verifikasi-kasubag.spm.kontrak.show', $spm->id) }}" class="btn btn-sm btn-primary">
                                        <i class="bi bi-pencil-square"></i> Verifikasi
                                    </a>
                                @else
                                    <a href="{{ route('verifikasi-kasubag.spm.kontrak.show', $spm->id) }}" class="btn btn-sm btn-outline-secondary">
