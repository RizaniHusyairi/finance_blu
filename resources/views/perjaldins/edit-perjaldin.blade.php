...
@php
    // Nilai default mekanisme pembayaran (enum bisa berupa instance atau string).
    $mekanismeAktif = $tagihan->mekanisme_pembayaran instanceof \App\Enums\MekanismePembayaran
        ? $tagihan->mekanisme_pembayaran->value
        : ($tagihan->mekanisme_pembayaran ?: \App\Enums\MekanismePembayaran::defaultFor('PERJALDIN')->value);
    $selectedMekanisme = old('mekanisme_pembayaran', $mekanismeAktif);

    $tanggalTtd = old('tanggal_ttd', $tagihan->tanggal_ttd ? \Carbon\Carbon::parse($tagihan->tanggal_ttd)->format('Y-m-d') : date('Y-m-d'));
@endphp
...
<div class="perj-hero">
    ...
    <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap">
        <div class="flex-grow-1">
            <span class="hero-tag"><i class="bi bi-pencil-square"></i> Form Edit Pengajuan</span>
            <h2><i class="bi bi-airplane me-2"></i>Perjalanan Dinas</h2>
            <p>Perbarui data dokumen, verifikator, dan daftar nominatif peserta. Dokumen: <strong>{{ $tagihan->nomor_tagihan }}</strong></p>
        </div>
...
<form action="{{ route('perjaldins.update-perjaldin', $tagihan->id) }}" method="POST" enctype="multipart/form-data" id="perjaldinForm">
    @csrf
    @method('PUT')
        ...
        <div class="sec-body">
            <div class="row g-3">
                <div class="col-md-5 col-lg-4">
                    ...
                    <input type="text" name="nomor_perjaldin" id="inp_nomor" class="form-control modern fw-bold text-primary font-monospace"
                           value="{{ old('nomor_perjaldin', $tagihan->nomor_tagihan) }}" required placeholder="KU.201/0001/APTP/{{ date('Y') }}">
                </div>
                <div class="col-md-7 col-lg-6">
                    <label class="form-label-modern">Uraian / Judul Perjalanan <span class="text-danger">*</span></label>
                    <input type="text" name="deskripsi" id="inp_deskripsi" class="form-control modern" placeholder="Contoh: Rapat Koordinasi Anggaran..." required value="{{ old('deskripsi', $tagihan->deskripsi) }}">
                </div>

                {{-- Periode & TTD memakai nilai existing (tidak ditampilkan di form). --}}
                <input type="hidden" name="periode_bulan" id="inp_bulan" value="{{ old('periode_bulan', $tagihan->periode_bulan ?: date('n')) }}">
                <input type="hidden" name="periode_tahun" id="inp_tahun" value="{{ old('periode_tahun', $tagihan->periode_tahun ?: date('Y')) }}">
                <input type="hidden" name="kota_ttd" id="inp_kota" value="{{ old('kota_ttd', $tagihan->kota_ttd ?: 'Samarinda') }}">
                <input type="hidden" name="tanggal_ttd" id="inp_tanggal" value="{{ $tanggalTtd }}">
                ...
                <div class="col-md-12 col-lg-6">
                    ...
                    <select name="mekanisme_pembayaran" class="form-select modern" required>
                        @foreach(\App\Enums\MekanismePembayaran::optionsFor('PERJALDIN') as $val => $lbl)
                            <option value="{{ $val }}" {{ $selectedMekanisme === $val ? 'selected' : '' }}>{{ $lbl }}</option>
                        @endforeach
                    </select>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="verif-mini vm-primary">
                        ...
                        <input type="hidden" name="ppk_nama_snapshot" id="ppkNamaSnapshot" value="{{ old('ppk_nama_snapshot', $tagihan->ppk_nama_snapshot) }}">
                        <div class="field-mini">
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="verif-mini vm-success">
                        ...
                        <input type="hidden" name="ppspm_nama_snapshot" id="ppspmNamaSnapshot" value="{{ old('ppspm_nama_snapshot', $tagihan->ppspm_nama_snapshot) }}">
                        <div class="field-mini">
                            <label>NIP</label>
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="verif-mini vm-info">
                        ...
                        <input type="hidden" name="koordinator_keuangan_nama_snapshot" id="koorKeuanganNamaSnapshot" value="{{ old('koordinator_keuangan_nama_snapshot', $tagihan->koordinator_keuangan_nama_snapshot) }}">
                        <div class="field-mini">
                            <label>NIP</label>
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="verif-mini vm-warning">
                        ...
                        <input type="hidden" name="bendahara_penerimaan_nama_snapshot" id="bendaharaPenerimaanNamaSnapshot" value="{{ old('bendahara_penerimaan_nama_snapshot', $tagihan->bendahara_penerimaan_nama_snapshot) }}">
                        <div class="field-mini">
                            <label>NIP</label>
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="verif-mini vm-danger">
                        ...
                        <input type="hidden" name="bendahara_pengeluaran_nama_snapshot" id="bendaharaNamaSnapshot" value="{{ old('bendahara_pengeluaran_nama_snapshot', $tagihan->bendahara_pengeluaran_nama_snapshot) }}">
                        <div class="field-mini">
                            <label>NIP <span class="text-danger">*</span></label>
        ...
        <div class="sec-head">
            <div class="head-left">
                ...
                <div>
                    ...
                    <small>Perbarui peserta perjalanan dinas dan rincian biaya</small>
                </div>
            </div>
        ...
        <div class="sec-body">
            ...
            <div id="pesertaRepeater">
                @php
                    $oldRaw = old('peserta');
                    if ($oldRaw !== null) {
                        $oldPeserta = collect($oldRaw)->map(function ($r) {
                            $r['id'] = $r['id'] ?? ($r['detail_id'] ?? null);
                            return $r;
                        })->all();
                    } else {
                        $oldPeserta = $tagihan->detailPerjaldin->map(function ($d) {
                            $arr = $d->toArray();
                            $arr['id'] = $d->id;
                            return $arr;
                        })->all();
    ...
    <div class="submit-bar-perj" id="submitBar">
        <div class="sb-status">
            ...
            <div>
                ...
                <p class="small text-muted mb-0" id="sbDesc">Isi semua field wajib pada Step A–C untuk menyimpan perubahan.</p>
            </div>
        </div>
