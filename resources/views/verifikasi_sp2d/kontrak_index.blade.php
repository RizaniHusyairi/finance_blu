@extends('layouts.app')
@section('title', 'Verifikasi SP2D Kontrak — ' . $currentRole)

@section('content')
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4">
    <div>
        <h5 class="fw-bold text-primary mb-0">Verifikasi SP2D</h5>
        <p class="text-muted mb-0">Kontrak — {{ $currentRole }}</p>
    </div>
</div>
    ...
    <div class="card-body py-3">
        <form method="GET" action="{{ route($routePrefix . '.index') }}" class="row g-2 align-items-end">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="search" class="form-control form-control-sm" placeholder="Nomor SP2D, NPI, SPM, SPP, SPK, vendor, pekerjaan..." value="{{ $search }}">
            </div>
            <div class="col-md-2 d-flex gap-1">
                ...
                <a href="{{ route($routePrefix . '.index') }}" class="btn btn-outline-secondary btn-sm"><i class="material-icons-outlined" style="font-size:16px; vertical-align: middle;">refresh</i></a>
            </div>
        </form>
    </div>
</div>
                ...
                <tbody>
                    @forelse($sp2dList as $idx => $sp2d)
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="fw-bold text-primary">{{ $sp2d->nomor_sp2d ?? 'Draft / Belum Bernomor' }}</span>
                            ...
                            <td>
                                <div style="font-size: 12px; line-height: 1.6;">
                                    ...
                                    <span class="text-muted">SPM:</span> {{ $spm?->nomor_spm ?? '-' }}<br>
                            ...
                            <td class="text-center">
                                @php($ps = $sp2d->_ppkApproval?->status)
                            ...
                            <td class="text-center">
                                @php($ks = $sp2d->_kasubbagApproval?->status)
                            ...
                            <td class="text-center">
                                ...
                                @else
                                    <a href="{{ route($routePrefix . '.show', $sp2d->id) }}" class="btn btn-sm btn-outline-secondary px-3">
