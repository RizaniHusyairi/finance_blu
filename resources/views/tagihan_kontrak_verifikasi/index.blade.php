...
{{-- Role Info Banner --}}
<div class="role-banner d-flex align-items-center gap-3 mb-4 py-2">
    <i class="bi bi-person-badge fs-4 text-primary"></i>
    <div class="small text-secondary">
        Anda login sebagai <strong class="text-dark">{{ $userRole }}</strong>.
        Dokumen yang ditampilkan adalah tagihan kontrak yang relevan untuk tahap verifikasi Anda.
    </div>
</div>

{{-- Tabs --}}
<div class="card card-modern-table border-0 shadow-sm">
        ...
        <div class="card-body p-0">
            @if($tagihansPerlu->isEmpty())
                <div class="text-center text-muted py-5">
                    ...
                    <p class="small text-muted">Tagihan akan muncul di sini setelah Pejabat Pengadaan mengajukan tagihan kontrak.</p>
                </div>
            @else
                    ...
                    <table class="table modern-table table-hover align-middle mb-0" id="tablePerlu">
                        <thead>
                            <tr>
                                ...
                                <th><i class="bi bi-briefcase"></i>Vendor & Pekerjaan</th>
                                <th><i class="bi bi-wallet2"></i>Total Bruto</th>
                                    ...
                                    <td>
                                        ...
                                        @if($termin)
                                            <span class="badge bg-light text-secondary border small mt-1">
                                                <i class="bi bi-tag me-1"></i>Termin {{ $termin->termin_ke ?? '-' }} ({{ $termin->jenis_termin }})
                                            </span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="fw-semibold text-truncate" style="max-width:260px;" title="{{ $kontrak->vendor->nama_pihak ?? '-' }}">
                                            <i class="bi bi-building me-1 text-primary"></i>{{ $kontrak->vendor->nama_pihak ?? '-' }}
                                        </div>
                                        <div class="text-muted small text-truncate" style="max-width:260px;" title="{{ $kontrak->nama_pekerjaan ?? '-' }}">
                                            {{ $kontrak->nama_pekerjaan ?? '-' }}
                                        </div>
                                        @if($kontrak)
                                            <span class="font-monospace-spk mt-1">
                                                {{ $kontrak->nomor_spk ?? '-' }}
                                            </span>
                                        @endif
                                    </td>
                            ...
                            @foreach($tagihansRiwayat as $i => $tagihan)
                                ...
                                <tr class="doc-row">
                                    ...
                                    <td>
                                        ...
                                        <div class="text-muted small text-truncate" style="max-width:260px;">{{ $kontrak->nama_pekerjaan ?? '-' }}</div>
                                    </td>
