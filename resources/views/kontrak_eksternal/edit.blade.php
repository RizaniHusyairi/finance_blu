...
<div class="page-content">
    <div class="kf-page-head mb-4 kf-reveal" style="--d:.01s;">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 position-relative" style="z-index:1;">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h5 class="fw-bold mb-1" style="letter-spacing:-.3px; color:#fff;">Edit Kontrak</h5>
                    <div style="color:rgba(255,255,255,.85); font-size:.85rem;">
                        {{ $kontrak->nomor_surat_pesanan }} — skema termin dibangun ulang selama kontrak masih DRAFT.
                    </div>
                    <div class="d-flex flex-wrap gap-2 mt-2">
                        <span class="kf-head-chip"><i class="bi bi-hash"></i> {{ $kontrak->nomor_surat_pesanan }}</span>
                        <span class="kf-head-chip"><i class="bi bi-cash-stack"></i> Rp {{ number_format((float) $kontrak->nilai_total_kontrak, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
            <a href="{{ route('kontrak-eksternal.show', $kontrak->id) }}" class="btn btn-light rounded-3 fw-bold">
    ...
    <form method="POST" action="{{ route('kontrak-eksternal.update', $kontrak->id) }}" enctype="multipart/form-data" data-kf-form>
        @csrf
        @method('PUT')
