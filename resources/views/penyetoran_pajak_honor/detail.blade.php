@extends('layouts.app')
@section('title', 'Detail Penyetoran Pajak Honorarium')
...
@push('css')
<style>
    .detail-hero { background: linear-gradient(135deg, #f0fdf4, #ecfdf5); border: 1px solid rgba(15,23,42,.08); padding: 1.5rem; margin-bottom: 1.5rem; position: relative; border-radius: .75rem; }
    .detail-hero::before { content: ""; position: absolute; left: 0; top: 0; width: 4px; height: 100%; background: #10b981; border-radius: .75rem 0 0 .75rem; }
    ...
    .action-card { border: 2px solid #10b981; background: #fcfffe; border-radius: 1rem; box-shadow: 0 .25rem 1rem rgba(16,185,129,.15); }
    .lampiran-item { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: .5rem; padding: .75rem 1rem; }
    .honor-table th { font-size: .72rem; text-transform: uppercase; letter-spacing: .04em; color: #64748b; }
    .honor-table td { font-size: .82rem; vertical-align: middle; }
</style>
@endpush

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <x-page-title title="Detail Penyetoran Pajak Honorarium" subtitle="Workspace billing &amp; NTPN PPh 21 honorarium" />
        <a href="{{ route('pajak-potongan.honor.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i> Kembali</a>
    </div>
    ...
    <div class="detail-hero p-4 rounded-3 shadow-sm">
        <div class="d-flex flex-column flex-xl-row justify-content-between gap-4">
            <div class="flex-grow-1">
                <h3 class="fw-bold mb-2 text-dark">PPh 21 Honorarium</h3>
                <div class="text-muted small mb-3">{{ Str::limit($tagihan?->deskripsi ?? 'Tagihan Honorarium', 70) }} &bull; {{ $tagihan?->nomor_tagihan ?? '-' }}</div>
                <div class="d-flex flex-wrap gap-2 mb-4 align-items-center">
                    <span class="badge {{ $statusClass }} px-3 py-2">{{ $statusSetor }}</span>
                    <span class="badge bg-light text-dark px-3 py-2 border">Honorarium</span>
                ...
                <div class="row g-3">
                    <div class="col-md-3 col-6"><div class="info-tile"><div class="label">Nominal PPh 21</div><div class="value text-danger fs-6">Rp {{ number_format($potongan->nominal_potongan, 0, ',', '.') }}</div></div></div>
                    <div class="col-md-3 col-6"><div class="info-tile"><div class="label">DPP (Σ Nilai Honor)</div><div class="value">Rp {{ number_format($potongan->dpp, 0, ',', '.') }}</div></div></div>
                    ...
                    <div class="col-md-3 col-6"><div class="info-tile"><div class="label">Penerima</div><div class="value">{{ $detailHonor->count() }} orang</div></div></div>
                </div>
            </div>
        </div>
    </div>
    ...
    @if(abs($selisihPph) >= 1)
        <div class="alert alert-warning border-0 shadow-sm d-flex align-items-center gap-2">
            ...
            <div class="small">Terdapat selisih antara total PPh per penerima dengan nominal potongan tersimpan sebesar <strong>Rp {{ number_format(abs($selisihPph), 0, ',', '.') }}</strong>. Periksa kembali data honorarium.</div>
        </div>
    @endif
        ...
        <div class="col-xl-7">
            ...
            <div class="card section-card mb-4">
                <div class="card-body p-4">
                    <div class="section-heading text-secondary"><i class="bi bi-file-earmark-text"></i> Informasi Tagihan Honorarium</div>
                    <div class="row g-3">
                        ...
                        <div class="col-12"><div class="info-block"><div class="label">Uraian</div><div class="value">{{ $tagihan?->deskripsi ?? '-' }}</div></div></div>
            ...
            {{-- Rincian Penerima & PPh --}}
            <div class="card section-card mb-4">
                <div class="card-body p-4">
                    <div class="section-heading text-secondary"><i class="bi bi-people"></i> Rincian Penerima &amp; PPh 21</div>
                    <div class="table-responsive">
                        <table class="table table-sm honor-table align-middle mb-0">
                            <thead class="table-light">
                                <tr>
                                    <th>Nama / NIP</th>
                                    <th>Jabatan</th>
                                    <th class="text-end">Nilai Honor</th>
                                    <th class="text-end">PPh 21</th>
                                    <th class="text-center">e-Bupot</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($detailHonor as $d)
                                    <tr>
                                        <td>
                                            <div class="fw-semibold text-dark">{{ $d->nama_personel }}</div>
                                            <div class="text-muted small">{{ $d->nrp_nip ?? '-' }}</div>
                                        </td>
                                        <td class="small">{{ $d->jabatan ?? '-' }}</td>
                                        <td class="text-end">Rp {{ number_format($d->nilai_honor, 0, ',', '.') }}</td>
                                        <td class="text-end text-danger">Rp {{ number_format($d->pph, 0, ',', '.') }}</td>
                                        <td class="text-center">
                                            @if($d->bupot_status === 'FINAL')
                                                <a href="{{ route('pajak-potongan.honor.bupot', $d->id) }}" target="_blank" class="btn btn-sm btn-outline-success py-0 px-2">
                                                    <i class="bi bi-file-earmark-check"></i> {{ $d->nomor_bupot }}
                                                </a>
                                            @else
                                                <span class="badge bg-light text-muted border">Draft</span>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="5" class="text-center text-muted py-3">Belum ada penerima honorarium.</td></tr>
                                @endforelse
                            </tbody>
                            <tfoot class="table-light">
                                <tr>
                                    <th colspan="2" class="text-end">TOTAL</th>
                                    <th class="text-end">Rp {{ number_format($detailHonor->sum('nilai_honor'), 0, ',', '.') }}</th>
                                    <th class="text-end text-danger">Rp {{ number_format($detailHonor->sum('pph'), 0, ',', '.') }}</th>
                                    <th></th>
                                </tr>
                            </tfoot>
                        </table>
            ...
            <div class="card section-card mb-4">
                <div class="card-body p-4">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4"><div class="info-block"><div class="label">Nominal</div><div class="value text-danger fw-bold">Rp {{ number_format($potongan->nominal_potongan, 0, ',', '.') }}</div></div></div>
                        <div class="col-md-6"><div class="info-block"><div class="label">Kode Billing</div><div class="value font-monospace text-primary">{{ $potongan->kode_billing ?? '—' }}</div></div></div>
                        <div class="col-md-6"><div class="info-block"><div class="label">NTPN</div><div class="value font-monospace text-success">{{ $potongan->ntpn ?? '—' }}</div></div></div>
                    </div>
                </div>
            </div>
            ...
            <div class="card section-card mb-4">
                <div class="card-body p-4">
                    ...
                    @forelse($potongan->arsipDokumen ?? collect() as $arsip)
            ...
            <div class="sticky-top" style="top: 1.5rem; z-index: 1;">
                ...
                <div class="card section-card mb-4">
                    <div class="card-body p-4">
                        ...
                        @php
                            $checks = [
                                ['label' => 'Tagihan honorarium valid', 'ok' => (bool) $tagihan],
                                ['label' => 'Potongan PPh 21 valid', 'ok' => $potongan->nominal_potongan > 0],
                ...
                {{-- FORM KODE BILLING --}}
                @if($canInputBilling)
                <div class="card action-card mb-4" style="border-color:#6366f1;">
                    ...
                    <div class="card-body p-4 bg-white rounded-bottom-3">
                        <form action="{{ route('pajak-potongan.honor.billing', $potongan->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label class="form-label small fw-semibold">E-Billing (Cetakan DJP) <span class="text-danger">{{ $arsipBilling ? '' : '*' }}</span></label>
                                <input type="file" name="file_billing" class="form-control form-control-sm" accept=".pdf,.jpg,.jpeg,.png" {{ $arsipBilling ? '' : 'required' }}>
                                <div class="form-text">PDF / JPG / PNG, maks 5MB.</div>
                                @if($arsipBilling)
                                    <div class="mt-1 small text-success"><i class="bi bi-paperclip"></i>
                                        <a href="{{ Storage::url($arsipBilling->path_file) }}" target="_blank" class="text-success fw-semibold">{{ $arsipBilling->nama_file_asli }}</a>
                ...
                <div class="card action-card mb-4">
                    ...
                    <div class="card-body p-4 bg-white rounded-bottom-3">
                        <form action="{{ route('pajak-potongan.honor.ntpn', $potongan->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
                ...
                <div class="alert alert-warning border-0 shadow-sm d-flex align-items-center gap-3 p-4 mb-4">
                    ...
                    <div>
                        ...
                        <div class="small">Upload bukti transfer pada menu SP2D terlebih dahulu sampai status SP2D EXECUTED dan tagihan SELESAI. Setelah itu Kode Billing dan NTPN dapat diinput.</div>
                    </div>
                </div>
                @endif

                {{-- Cetak ringkasan --}}
                <a href="{{ route('pajak-potongan.honor.cetak', $potongan->id) }}" target="_blank" class="btn btn-outline-secondary w-100"><i class="bi bi-printer me-1"></i> Cetak Ringkasan Penyetoran</a>
            </div>
        </div>
    </div>
@endsection
