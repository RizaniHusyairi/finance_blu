@extends('layouts.app')
@section('title', 'Buku Pengesahan Pendapatan')

@include('pembukuan.partials.styles')

@section('content')
    <x-page-title title="Pembukuan" subtitle="Buku Pengesahan Pendapatan" />

    <div class="book-hero">
        <div class="d-flex flex-column flex-lg-row justify-content-between gap-3 align-items-lg-center">
            <div>
                <h4 class="mb-1 fw-bold text-dark">Buku Pengesahan Pendapatan</h4>
                <div class="text-muted">Laporan pengesahan pendapatan BLU per periode berdasarkan `laporan_pengesahan_blu` sisi penerimaan.</div>
            </div>
            <div class="d-flex flex-wrap gap-2">
                ...
                <a href="{{ route('pembukuan.pengesahan-pendapatan.pdf', request()->query()) }}" target="_blank" class="btn btn-outline-danger">
    ...
    @php
        $cards = [
            ...
            ['label' => 'Pendapatan Disahkan (Rp)', 'value' => 'Rp ' . number_format($summary['total_pendapatan_disahkan'] ?? 0, 0, ',', '.'), 'class' => 'text-success', 'hint' => 'Total penerimaan dari laporan berstatus DISAHKAN sesuai filter.'],
        ];
    @endphp
    @include('pembukuan.partials.summary-cards', ['cards' => $cards])

    <div class="book-filter">
        <form method="GET" action="{{ route('pembukuan.pengesahan-pendapatan.index') }}" class="row g-3 align-items-end">
            <div class="col-md-3">
            ...
            <div class="col-md-2 d-flex gap-2">
                ...
                <a href="{{ route('pembukuan.pengesahan-pendapatan.index') }}" class="btn btn-outline-secondary">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="card book-card">
        ...
        <div class="card-body p-0">
            @if($reports->isEmpty())
                @include('pembukuan.partials.empty-state', ['title' => 'Belum ada laporan pengesahan', 'message' => 'Tidak ada laporan pengesahan BLU yang sesuai filter. Gunakan tombol "Buat Laporan Periode" untuk membuat laporan dari data BKU.'])
            @else
                        ...
                        <tbody>
                            @foreach($reports as $report)
                                <tr>
                                    ...
                                    <td class="text-end fw-bold">Rp {{ number_format($report->total_penerimaan, 0, ',', '.') }}</td>
                                    <td class="text-end">Rp {{ number_format($report->saldo_akhir_blu, 0, ',', '.') }}</td>
                                    <td>@include('pembukuan.partials.status-badge', ['value' => $report->status_pengesahan])</td>
                                    <td>{{ $report->status_sp3b ?? '-' }}</td>
                                    <td class="text-center"><a href="{{ route('pembukuan.pengesahan-pendapatan.show', $report->id) }}" class="btn btn-sm btn-outline-primary">Detail</a></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
