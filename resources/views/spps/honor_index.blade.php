...
@section('content')
    ...
    @php
        $belumSpp = $honorariums->where('status', 'DISETUJUI_PPK')->count();
        $prosesSpp = $honorariums->where('status', 'PROSES_SPP')->count();
        $selesaiSpp = $honorariums->where('status', 'SPP_TERBIT')->count();
        $tertahan = $honorariums->filter(fn ($item) => $item->spps->isNotEmpty() && optional($item->spps->first())->status_spp === 'Revisi')->count();
    @endphp
    ...
    <div class="card">
        <div class="card-body">
            <h6 class="mb-3">Daftar Honorarium Siap SPP</h6>
                    ...
                    <tbody>
                        @foreach ($honorariums as $idx => $hon)
                            <tr>
                                <td>{{ $idx + 1 }}</td>
                                <td>
                                    <strong>{{ $hon->nomor_tagihan }}</strong><br>
                                    <small>{{ Str::limit($hon->deskripsi, 60) }}</small>
                                </td>
                                <td>
                                    @if($hon->waktu_verifikasi_ppk)
                                        <strong>{{ $hon->waktu_verifikasi_ppk->format('d M Y') }}</strong><br>
                                        <small class="text-muted">{{ $hon->waktu_verifikasi_ppk->format('H:i') }}</small>
                                    @else
                                        <small class="text-muted">-</small>
                                    @endif
                                </td>
                                <td class="text-end">
                                    <strong>Rp {{ number_format($hon->total_netto, 0, ',', '.') }}</strong>
                                </td>
                                <td class="text-center">
                                    <span class="badge bg-light border text-dark">{{ $hon->detailHonorarium->count() }} Orang</span>
                                </td>
                                <td class="text-center">
                                    @if($hon->status === 'DISETUJUI_PPK' && $hon->spps->isEmpty())
                                        <span class="badge bg-warning text-dark">Belum Dibuat SPP</span>
                                    @elseif($hon->status === 'PROSES_SPP' || $hon->spps->isNotEmpty())
                                        <span class="badge bg-info">SPP Dibuat</span>
                                    @elseif($hon->status === 'SPP_TERBIT')
                                        <span class="badge bg-success">SPP Terbit</span>
                                    @else
                                        <span class="badge bg-secondary">{{ $hon->status }}</span>
                                    @endif
                                </td>
                                <td class="text-center">
